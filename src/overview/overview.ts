...
const UNIT_COLS: Col[] = [
  ...
  { key: 'fac',   label: 'Faction', val: d => F(d.faction),  cell: d => `<td class="${d.faction ? 'fac sig' : 'fac'}">${F(d.faction)}</td>` },
  { key: 'cost',  label: 'Cost',  num: true, val: d => d.cost || 0,  cell: d => `<td class="num">${d.cost}</td>` },
  { key: 'hp',    label: 'HP',    num: true, val: d => d.hp,         cell: d => `<td class="num">${d.hp}</td>` },
  { key: 'dmg',   label: 'Dmg',   num: true, val: d => d.dmg || 0,   cell: d => `<td class="num">${d.dmg || '–'}</td>` },
  { key: 'dps',   label: 'DPS',   num: true, val: d => dpsOf(d),     cell: d => `<td class="num">${dpsOf(d) ? dpsOf(d).toFixed(1) : '–'}</td>` },
  { key: 'rof',   label: 'ROF',   num: true, val: d => d.rof || 0,   cell: d => `<td class="num">${d.dmg > 0 ? d.rof + 's' : '–'}</td>` },
  { key: 'range', label: 'Range', num: true, val: d => d.range || 0, cell: d => `<td class="num">${d.range || '–'}</td>` },
  { key: 'speed', label: 'Speed', num: true, val: d => d.speed || 0, cell: d => `<td class="num">${d.speed || '–'}</td>` },
  { key: 'notes', label: 'Notes', val: () => '',               cell: d => `<td class="desc">${unitDesc(d)}</td>` },
];
let sortKey = '', sortDir = 1; // '' = original data order
let curFaction = 'all';

function buildUnits(faction: string) {
  curFaction = faction;
  let list = Object.keys(UNITS).filter(t => !(UNITS[t] as any).internal).map(t => UNITS[t] as any)
    .filter(d => faction === 'all' || !d.faction || d.faction === faction); // universal always shows
  if (sortKey) {
    const col = UNIT_COLS.find(c => c.key === sortKey)!;
  ...
  document.getElementById('unitHead')!.innerHTML = '<tr>' + UNIT_COLS.map(c => {
    const arrow = sortKey === c.key ? (sortDir > 0 ? ' ▲' : ' ▼') : '';
    return `<th class="sortable" data-k="${c.key}"${c.num ? ' style="text-align:right"' : ''}>${c.label}${arrow}</th>`;
  }).join('') + '</tr>';
  document.getElementById('unitBody')!.innerHTML = list.map(d => '<tr>' + UNIT_COLS.map(c => c.cell(d)).join('') + '</tr>').join('');
  document.querySelectorAll('#unitHead th.sortable').forEach(th => th.addEventListener('click', () => {
    const k = (th as HTMLElement).dataset.k!;
    if (sortKey === k) sortDir = -sortDir; else { sortKey = k; sortDir = 1; }
    buildUnits(curFaction);
  }));
}
